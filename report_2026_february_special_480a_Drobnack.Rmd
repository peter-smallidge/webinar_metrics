...
author: "Revisions to 480a, by Jason Drobnack 25 February 2026"
date: "`r Sys.Date()`"
...
#   "C:\\Users\\pjs23\\Documents\\Projects\\CCE\Webinar Data and Files\\webinar evaluation montly summary\\2026.data.attend.exit\\2026_02_special_480a_Drobnack\\datafile_480a_feb2026.xlsx"

# INSTRUCTIONS
# 
# create a worksheet FROM THE ZOOM OUTPUT that is cleaned with the following features
# a. name the worksheet "full_cleaned" for attendee data and "registration" for registration data. 
# Manually condense users with multiple entries. 
#
# (note 6/19/2025 - it isn't clear why I need a separate file for "registration". That data is duplicative with 
...
#  and a file of the participation.  This R code will likely need to be modified to "join" those data files.)

# b. retain attended and those who didn't attend

# c. select the join and leave columns. Right click and make the column type "numeric" with 8 decimal places.


# d. (revised 6/19/2025) column names: attended name first last email postal state join leave duration perspective ceu acres notification
...
# create a worksheet FROM THE QUALTRICS OUTPUT that is named "survey_cleaned"
...
#file path = "C:\\Users\\pjs23\\Documents\\Projects\\CCE\\Webinar Data and Files\\webinar evaluation montly summary\\2026.data.attend.exit\\2026_02_ecological_forestry_Catanzaro\\data_for_analysis_Catanzaro.xlsx"
#
#

file_path_data <- "C:\\Users\\pjs23\\Documents\\Projects\\CCE\\Webinar Data and Files\\webinar evaluation montly summary\\2026.data.attend.exit\\2026_02_special_480a_Drobnack\\datafile_480a_feb2026.xlsx"


webinar_name <- "2026_Feb_25_Updated_480a_Regulations_for_Owners_J.Drobnack" 

output_folder <- "C:\\Users\\pjs23\\Documents\\Projects\\CCE\\Webinar Data and Files\\webinar evaluation montly summary\\2026.data.attend.exit\\2026_02_special_480a_Drobnack"
...
input_zoom <- full_join(input_zoom_register, input_zoom_attend, by = ("email")) %>% 
    select(attended, first, last, name, email,perspective, state, join, leave, duration,
           acres, enrolled) %>% 
...
CEU_1 <- input_zoom %>% 
    select( attended, email, first, last, state, join, leave, duration, perspective) %>%
    filter(str_detect(perspective, "Forester")) %>% 
    filter(! attended == "No") %>% 
    arrange(perspective, last) %>% 
    select(email, first, last,  state, join, leave, duration, perspective)
...
summ_perspective1 <- input_zoom %>% 
  mutate(perspective = if_else(is.na(perspective), "Forester", perspective)) %>%
...
summ_perspective2 <- bind_rows(summ_perspective1, missing_perspectives) %>% 
    filter(!perspective %in% c("Agency or Institution", "Natural Resources Specialist")) %>%
    group_by(perspective) %>% 
    summarize(
...
acres_plot <- ggplot(acres_freq, aes(x = acres_bracket, y = n)) +
  ...
  theme_minimal() +
  labs(title = "Frequency Distribution by Acres",
       x = "Acres Bracket",
       y = "Count") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
[webinar archived at] https://www.youtube.com/watch?v=r4fL8D2LO8I
<!-- Create summary of General Comments -->
